...
async function openChatWindow(noun) {
    ...
    const initialResponse = await fetchGPTDefinition(noun);
    // HTML content for the webview
    panel.webview.html = getWebviewContent(noun, initialResponse);
    // Handle messages from the webview
    panel.webview.onDidReceiveMessage(async (message) => {
        if (message.command === 'askGPT') {
            const response = await fetchGPTAnswer(noun, message.text);
